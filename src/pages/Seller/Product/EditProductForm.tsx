...
function EditProductForm() {
  ...
  const { user } = useUser();
  const { uploadProductPhoto, updateShopProduct } = useShop();
  ...
  const submitForm = async () => {
    setDisableBtn(true);
    try {
      var productPayload: IEditProductShopPayload = {
        id: Number(id),
        name: "",
        description: "",
      ...
      productPayload.is_active = formInput.isActive === "true" ? true : false;
      ...
      const response = await updateShopProduct(productPayload);
  ...
  const setAllForm = (product: IProductPayload) => {
    const isDefaultVariant: boolean =
      !!product?.variant_group?.name.includes("DEFAULT");
    const defaultPrice = isDefaultVariant
      ? product?.variant_group?.variant_types[0].price.toString() ?? ""
      : "";
    const defaultStock = isDefaultVariant
      ? product?.variant_group?.variant_types[0].stock.toString() ?? ""
      : "";
    ...
    setFormInput({
      ...
      minQty: product?.min_buy_qty?.toString() ?? "",
      maxQty: product?.max_buy_qty?.toString() ?? "",
      weight: product?.weight?.toString() ?? "",
      dangerousGoods: product?.is_hazardous ? "true" : "false",
      condition: product?.condition?.toString() ?? "",
      SKU: product?.internal_sku?.toString() ?? "",
      isActive: product?.is_active ? "true" : "false",
    });

    const primaryCat = categories.find(
      (cat) => cat.name === product?.category_detail?.primary_category?.name
    );
    const secondaryCat = primaryCat?.secondary_category?.find(
      (cat) => cat.name === product?.category_detail?.secondary_category?.name
    );
    const tertiaryCat = secondaryCat?.tertiary_category?.find(
      (cat) => cat.name === product?.category_detail?.tertiary_category?.name
    );
    ...
    product.product_photos?.forEach((photo) => {
      convertImgUrlToFile(photo.url);
    });

    if (!isDefaultVariant) {
      setVariation(true);

      const vg = product.variant_group?.name.split(",").map((val) => {
        return {
          vg_name: val,
        } as ICreateVariantGroup;
      });
      setVgInput(vg ?? []);

      const vl = product.variant_group?.variant_types.map(
        (val) =>
          ({
            vtOne_name: val.name.split(",")[0] ?? "",
            vtTwo_name: val.name.split(",")[1] ?? "",
  ...
  const convertImgUrlToFile = (url: string) => {
      ...
      <Box width="100%">
        ...
        >
          ...
          <VStack justifyContent={"center"} alignItems={"center"} spacing={8}>
            ...
            <Flex flexDirection={"row"} width="70%">
              <Text width={"30%"}>Status</Text>
              <Switch
                me={2}
                colorScheme="teal"
                size="lg"
                alignItems={"start"}
                isChecked={formInput.isActive === "true" ? true : false}
                onChange={() => {
                  setFormInput((val) => ({
                    ...val,
                    isActive: val.isActive === "true" ? "false" : "true",
                  }));
                }}
              />
              <Text fontWeight={"semibold"}>
                {formInput.isActive === "true" ? "Active" : "Not Active"}
              </Text>
            </Flex>
          </VStack>
        </Flex>
